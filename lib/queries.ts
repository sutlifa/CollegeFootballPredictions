...
export async function saveBracketRoundPicks(
  ...
  await sql.begin(async (tx) => {
    for (const { slot, teamId } of picks) {
      ...
      // Submitting a round ALWAYS clears the rounds after it, whether or
      // not any winner actually moved. This is a deliberate product rule,
      // not an oversight: re-opening a round means the bracket past that
      // point is being reconsidered, and a later-round pick that merely
      // happens to still be legal is not the same as one someone chose
      // under the current bracket. You re-pick from here.
      //
      // It was briefly made conditional on the pick having changed, which
      // did spare untouched branches -- but it also let a stale pick ride
      // through an edit, which is exactly what this is meant to prevent.
      // Do not reintroduce that without asking.
